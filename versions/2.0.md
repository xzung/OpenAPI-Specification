...
Swagger  是一个用于描述并文档化RESTful APIs 的工程。

The Swagger specification defines a set of files required to describe such an API. These files can then be used by the Swagger-UI project to display the API and Swagger-Codegen to generate clients in various languages. Additional utilities can also take advantage of the resulting files, such as testing tools.
Swagger规范 为了描述一个API,定义了一组文件。这些文件能够被Swagger-UI 工程使用去显示API ,并且Swagger-Codegen 能够利用它们生成不同语言的客户端。
附加实用工具能够利用结果文件， 比如测试工具。
...
Path  templating 指的是使用大括号去标记一个URL path 的部分，以替代使用path 参数。
...
一个和swagger 规格一致的描述RESTful API 的文档，可以使用YAML 或 JSON 来描述。
swagger specification 使用json 对象来描述RESTful API ； YAML是JSON 的一个超集 , 因此 ，YAML 也能够被用于描述一个 swagger specification 文件
...
模式（json 模式或 YAML 模式） 支持两种类型的字段： Fixed fields , Patterned fields

前者声明一个字段名 ， 后者为field name 声明一个正则模式.   Patterned  fields  可以有多种名称 ，只要它们在schema 中使唯一的field name.

ps: 这里的模式是指基于json描述OpenAPI specification的规则。更多模式的理解可以参考记事《json schema和json两个词汇在实际使用中的区别》

### File Structure  (json) 文件结构
...
api 的swagger描述数据都写到一个单独的文件中，依照惯例，这个swagger specification file 被命名为swagger.json

两个部分 ：swagger 描述部分 ，swagger 定义部分 

描述部分是 swagger.json , 定义部分呢？ 是代码中的swagger 规格注释吗？ 
...
在swagger specifiation中的元数据类型是基于 JSON-Schema Draft 4 的，  模型（Models） 使用Schema Object 来进行描述
Schema Object 是的一个JSON Schema Draft 4 子集。

一个附加的元数据类型“file”被 Parameter Object 用于描述参数类型，被response object 用于描述响应一个什么文件

元数据有一个可选的修饰属性 ： format . swagger 使用一些已知的格式去更为细致的定义哪些被使用的数据类型。 

无论怎样，format 属性是一个开放的string 值属性。它可以是任何documentaion 需要的值。 
例如 “email”,"uuid" 等等都可以被使用 ，即使swagger specifiation 中没有定义它们。
...
swagger specification 的几种object：

1.Schema Object 
2.Parameter Object   用于设置api 参数类型
3.Response Object
...
json 是一种对象 ，即 object  ; 一个json 字段也可以理解为一个对象，如：
{
   parameter:{ ...},
   response:{ ... }   
}

整个可以看做一个大的json 对象 （schema object ? 待确认）； parameter部分就可以理解为 parameter 对象（object） ,response 同理

因此，在swagger specification 中所有包含数组元素的字段都可以看做一个object. 对象名称用字段名，如info object  ,parameter object

整个swagger specification 文档都是讲解我们如何去定义一个合乎规范的json 文件 ： 哪些字段是必须的， 哪些字段位于整个json schema
的哪一层 。
